...
class HistorialOvocitoViewSet(viewsets.ModelViewSet):
    queryset = HistorialOvocito.objects.select_related('ovocito', 'paciente', 'usuario').all()
    serializer_class = HistorialOvocitoSerializer
    ...
    def get_queryset(self):
        ...
        ovocito = self.request.query_params.get('ovocito')
        if paciente:
            qs = qs.filter(paciente_id=paciente)
        if ovocito:
            qs = qs.filter(ovocito_id=ovocito)
